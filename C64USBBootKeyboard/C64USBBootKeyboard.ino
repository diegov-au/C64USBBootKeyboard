...
// 
// Updated to use hid-project library for support with BMC64 by diegov-au
//
...
int Row=0;
int i;
int HybridKeyboard=0;                                                         // Select 0 for normal or 1 for the left HybridKey allowing all f keys and cursor keys in windows mode. (Also has a shifted restore key)
...
KEY_1,         KEY_TILDE,         KEY_TAB,   KEY_2,     KEY_SPACE,               MODIFIERKEY_CTRL, KEY_Q,          KEY_ESC,   KEYPAD_8, KEYPAD_SLASH,            //  1     BS     CTRL 2    SPC   C=  Q   RunStop Joy2Up    Joy1Up
};
      ...
      if ((millis()-lastDebounceTime[keyPos])>debounceDelay) // debounce for each key individually
      {
        if ((InputValue == LOW) && keyDown[keyPos]==0) // if a key is pressed and wasn't already down
        {
          ...
          if (HybridKeyboard==1)
          {
            ...
            //BootKeyboard.press(keyDown[keyPos]);    // pass the keypress to windows
             BootKeyboard.press(KeyboardKeycode(keyDown[keyPos])); 
        ...
        if ((InputValue == HIGH) && keyDown[keyPos]!=0) // key is up and a character is stored in the keydown position
        {
            ...
            //BootKeyboard.release(keyDown[keyPos]);    // pass key release to windows
           BootKeyboard.release(KeyboardKeycode(keyDown[keyPos])); 
